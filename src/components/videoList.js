...
const FeedPage = () => {
  ...
  >
    ...
    >
      <IconButton sx={{ color: "#1e90ff" }} onClick={() => handleLike(currentIndex)}>
        <FavoriteIcon />
      </IconButton>
      <Typography sx={{ color: "#fff", fontSize: "0.9rem", textAlign: "center" }}>
        {likes[currentIndex] || 0}
      </Typography>
      <IconButton sx={{ color: "#1e90ff" }} onClick={handleCommentClick}>
        <CommentIcon />
      </IconButton>
      <IconButton sx={{ color: "#1e90ff" }} onClick={handleDownload}>
        <DownloadIcon />
      </IconButton>
      <IconButton
    ...
    <Dialog open={isCommentDialogOpen} onClose={() => setIsCommentDialogOpen(false)}>
      <DialogTitle sx={{ color: "#1e90ff" }}>Add a Comment</DialogTitle>
      <DialogContent>
        <TextField
          label="Comment"
          variant="outlined"
